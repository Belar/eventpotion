		...
		<div class="col-md-8 table-responsive">
			...
			@foreach($events as $event)
			  <tr>
				...
				<td class="hidden-xs"><span data-toggle="tooltip" data-placement="left" data-original-title="{{{ date('H:i', strtotime($event->start_date)) }}}">{{{ date("d", strtotime($event->start_date)) }}} - {{{ date("d M y", strtotime($event->finish_date)) }}}</span></td>
