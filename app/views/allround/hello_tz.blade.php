		...
		<div class="col-md-8 table-responsive">
			...
			@foreach($events as $event)
			  <tr>
				...
				<td class="hidden-xs"><span data-toggle="tooltip" data-placement="left" data-original-title="{{ Timezone::convert($event->start_date, Sentry::getUser()->timezone, 'H:i') }}">{{ Timezone::convert($event->start_date, Sentry::getUser()->timezone, 'd M') }} - {{ Timezone::convert($event->finish_date, Sentry::getUser()->timezone, 'd M \'y') }}</span></td>
				...
				<td class="hidden-xs">				
					...
					@if($event->brackets)
						<a href="{{{ $event->brackets }}}"><i class="fa fa-table"></i></a>
					@endif					
				</td>
